...
DEFAULT_OUTPUT_DIR = ROOT / "output" / "faitheval"
DEFAULT_LOG_DIR = ROOT / "logs" / "faitheval"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run agentic RAG on FaithEval Unanswerable with oracle context. "
            "This script is isolated under evaluation/faitheval and does not modify project code."
        )
    )
    parser.add_argument(
        "--dataset",
        ...
        help=f"FaithEval Unanswerable parquet path. Defaults to {DEFAULT_DATASET_PATH}.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output JSONL path. Defaults to output/faitheval/unanswerable_oracle_<critic>_<timestamp>.jsonl.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable query decomposition in the agentic query processor. "
            "Defaults to disabled to isolate the answer-critique effect."
        ),
    )
    parser.add_argument(
        "--llm-config",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Workflow max attempts. The runner uses a single-pass reasoner, so this is a guardrail.",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional log path. Defaults to logs/faitheval/unanswerable_oracle_<timestamp>.log.",
    )
...
class OracleRetriever:
    ...
    def run(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        qid = str(self.sample.get("qid") or self.sample.get("id") or "")
        context = str(self.sample.get("context") or "").strip()
        evidences: list[EvidenceItem] = []
        if context:
            evidences.append(
                EvidenceItem(
                    evidence_id=f"oracle:{qid or node_id}",
                    content=context,
                    source_type="text",
                    source_id="FaithEval-unanswerable-v1.0",
                    metadata={
                        "qid": qid,
                        "node_id": node_id,
                        "oracle_context": True,
...
def build_output_path(
    ...
    *,
    ...
    if mode == "naive":
        return DEFAULT_OUTPUT_DIR / f"unanswerable_oracle_naive_{run_id}.jsonl"
    critic_label = "critic_on" if enable_answer_critique else "critic_off"
    return DEFAULT_OUTPUT_DIR / f"unanswerable_oracle_agentic_{critic_label}_{run_id}.jsonl"
...
def run_sample(
    *,
    ...
    try:
        ...
        initial_state = create_initial_state(query=question, max_attempts=max_attempts)
...
def build_payload(
    *,
    ...
    return {
        "dataset": "FaithEval-unanswerable-v1.0",
...
def main() -> int:
    ...
    log_path = args.log or DEFAULT_LOG_DIR / f"unanswerable_oracle_{run_id}.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    configure_logging(log_path=log_path)

    samples = load_samples(args.dataset, start=args.start, limit=args.limit)[:2]
    ...
    if args.max_workers == 1:
        for ordinal, (offset, sample) in enumerate(indexed_samples, start=1):
            question = str(sample.get("question") or "")
            print(
                f"[{ordinal}/{total}] qid={sample.get('qid', '')} "
                f"mode={args.mode} critic={args.enable_answer_critique} {question}",
                flush=True,
            )
            record = run_sample(
                sample_index=offset,
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for future in as_completed(future_to_sample):
                ...
                print(
                    ...
                    f"qid={sample.get('qid', '')} mode={args.mode} "
                    f"critic={args.enable_answer_critique} "
